...
class User(Base):
    ...
    genetic_data = relationship("GeneticData", back_populates="user", cascade="all, delete-orphan")
    biometric_entries = relationship("BiometricEntry", back_populates="user", cascade="all, delete-orphan")
...
class GeneticData(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    dna_data = Column(Text, nullable=False)
    traits = Column(Text)  # JSON string
    health_risks = Column(Text)  # JSON string
    recommendations = Column(Text)  # JSON string
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    
    user = relationship("User", back_populates="genetic_data")
