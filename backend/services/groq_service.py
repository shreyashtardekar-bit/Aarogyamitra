...
async def generate_ai_content(prompt: str) -> str:
    ...
    try:
        ...
        if "authentication" in error_msg.lower() or "api key" in error_msg.lower():
            ...
            print(f"DEBUG - Groq Unexpected Error: {error_msg}")
            return f"⚠️ AI Service Error: {error_msg[:100]}. Please contact support if this persists."

async def get_workout_plan(prompt: str) -> str:
    return await generate_ai_content(f"Create a detailed workout plan based on this request: {prompt}")

async def get_meal_plan(prompt: str) -> str:
    return await generate_ai_content(f"Create a detailed meal plan based on this request: {prompt}")
